...
const ActivityCard = ({ booking, type, isPending, onAction }: { 
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl ${type === 'arrival' ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary'}`}>
            <User size={18} />
          </div>
          <div>
        ...
        <div className="flex flex-col items-end gap-2">
          <span className={`badge badge-sm font-bold ${
            ...
            booking.status === 'checked_in' ? 'badge-success' : 'badge-neutral'
          }`}>
      ...
      <div className="mt-4 pt-4 border-t border-base-200 flex items-center justify-between">
        <div className="flex items-center gap-1 text-xs text-base-content/40">
          <Clock size={12} />
          <span>{type === 'arrival' ? 'Check-in from 14:00' : 'Check-out by 11:00'}</span>
        </div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h2 className="text-xl font-bold tracking-tight flex items-center gap-2">
            Arrivals Today
            <span className="bg-primary/10 text-primary px-2 py-0.5 rounded-lg text-xs font-black">{arrivals.length}</span>
          </h2>
        </div>
        ...
        <div className="space-y-4">
          <AnimatePresence mode='popLayout'>
            {arrivals.length === 0 ? (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-base-content/40 text-center py-10 bg-base-100/50 rounded-3xl border border-dashed border-base-300">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h2 className="text-xl font-bold tracking-tight flex items-center gap-2">
            Departures Today
            <span className="bg-secondary/10 text-secondary px-2 py-0.5 rounded-lg text-xs font-black">{departures.length}</span>
          </h2>
        </div>
