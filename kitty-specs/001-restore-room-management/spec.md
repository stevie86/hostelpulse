...
- **FR-08:** "Soft Delete" mechanism.
  - _Implementation Note:_ Since `deletedAt` is not in schema yet, use `status = 'archived'` (or add column if needed during task phase).
  - _Decision:_ For this spec, we will treat "Deletion" as checking for dependencies (bookings). If no bookings, hard delete. If bookings, prevent delete (or archive).
