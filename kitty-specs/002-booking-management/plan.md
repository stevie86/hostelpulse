...
- [x] **Task 1:** Implement `AvailabilityService` (logic to check overlaps).
  - _Sub-task:_ Implement `findAvailableBedLabel(roomId, checkIn, checkOut)` helper.
...
- [x] **Task 6:** Create `BookingForm` (Multi-step).
  - _Note:_ Will use simple inputs for Guest initially until Feature 003 is fully ready.
...
_For dorms, we count these overlaps. If count < room.beds, we allow it._
