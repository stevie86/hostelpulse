...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      const { data, error } = await supabaseServer.from('guests').select('*').order('created_at', { ascending: false })
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supabaseServer.from('guests').insert({ name, email, phone, nationality }).select('*').single()
      if (error) return res.status(500).json({ error: error.message })
